...
export function CampusMap({ highlightedPath = [], source, destination }: CampusMapProps) {
  ...
  // Check if route has segments on other floors
  const hasOtherFloorSegments = useMemo(() => {
    if (highlightedPath.length === 0) return false;
    return highlightedPath.some(id => {
      const loc = getLocation(id);
      return loc && loc.floor !== activeFloor;
    });
  }, [highlightedPath, activeFloor]);
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="grid grid-cols-4 gap-2 rounded-xl border-2 border-foreground bg-secondary/30 p-1.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
        {[0, 1, 2, 3].map((floor) => (
          <button
            ...
            className={`flex h-11 items-center justify-center rounded-lg text-xs sm:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer border-2 ${
              activeFloor === floor
      ...
      {/* Floating Walk Info Card */}
      {highlightedPath.length > 0 && (
        <div className="absolute bottom-3 left-3 right-3 sm:right-auto flex flex-col gap-1.5 rounded-lg bg-white/95 px-4 py-3 text-sm shadow-xl border border-gray-200 backdrop-blur-sm max-w-xs">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <svg className="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 24 24">
                <path d="M13.5 5.5c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zM9.8 8.9L7 23h2.1l1.8-8 2.1 2v6h2v-7.5l-2.1-2 .6-3C14.8 12 16.8 13 19 13v-2c-1.9 0-3.5-1-4.3-2.4l-1-1.6c-.4-.6-1-1-1.7-1-.3 0-.5.1-.8.1L6 8.3V13h2V9.6l1.8-.7"/>
              </svg>
              <span className="font-semibold text-gray-800">Route Info</span>
            </div>
            <div className="h-4 w-px bg-gray-300" />
            <div className="flex items-center gap-1">
              <span className="font-semibold text-gray-900">
                {Math.round(highlightedPath.length * 4)}m
              </span>
            </div>
          </div>
          {hasOtherFloorSegments && (
            <p className="text-[11px] font-medium text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-200/50">
              ⚠ Route spans multiple floors. Click elevator tabs to view other floors.
            </p>
          )}
        </div>
      )}
