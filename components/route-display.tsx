...
export function RouteDisplay({ result, source, destination }: RouteDisplayProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Route Summary Card */}
      <div className="border-2 border-foreground bg-card p-6 rounded-lg">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2 bg-green-500/10 px-4 py-2 rounded-full border border-green-500/30">
            ...
            <span className="font-medium">{getLocationName(source)}</span>
          </div>
          ...
          <div className="flex items-center gap-2 bg-red-500/10 px-4 py-2 rounded-full border border-red-500/30">
            ...
            <span className="font-medium">{getLocationName(destination)}</span>
          </div>
        </div>
      </div>

      {/* Campus Map with Animated Route */}
      <div className="border-2 border-foreground bg-card p-4 sm:p-6 rounded-lg">
        <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-foreground flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border-2 border-primary bg-gradient-to-br from-primary/5 to-primary/15 p-5 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <Footprints className="h-4 w-4 text-primary" />
            <span className="text-xs font-semibold uppercase tracking-wide text-primary">Distance</span>
          </div>
          <p className="text-3xl font-bold text-foreground">{result.totalDistance}<span className="text-lg ml-1 text-muted-foreground">m</span></p>
        </div>
        <div className="border-2 border-foreground/50 bg-gradient-to-br from-secondary to-secondary/50 p-5 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <Clock className="h-4 w-4 text-muted-foreground" />
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Walking Time</span>
          </div>
          <p className="text-3xl font-bold text-foreground">~{Math.ceil(result.totalDistance / 80)}<span className="text-lg ml-1 text-muted-foreground">min</span></p>
        </div>
      </div>
      ...
      <div className="border-2 border-foreground bg-card p-6 rounded-lg">
        ...
        <div className="flex flex-col gap-0">
          {meaningfulSteps.map((step, index) => (
            ...
            >
              ...
              <div className="flex flex-col gap-1 pt-1">
                <p className="font-medium text-foreground">{step.direction}</p>
                <p className="text-sm text-muted-foreground flex items-center gap-1">
                  <span className="inline-block px-1.5 py-0.5 bg-secondary rounded text-xs font-medium">
                    {step.distance}m
                  </span>
          ...
          <div className="flex items-start gap-4 pl-4">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 border-green-500 bg-green-500 text-sm font-bold text-white">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <div className="flex flex-col gap-1 pt-1">
              <p className="font-medium text-green-600">Destination Reached!</p>
